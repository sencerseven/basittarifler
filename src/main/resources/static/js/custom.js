(function ($) {
    ...
    $('.add-button.add-ing').on("click",function(event){
        ...
        var newMajesticItem= '<li style="display: none">' +
            ...
            ' <input type="text" name="ingredientDetails[]" id="ingredientDetails"/>' +
            ' <span class="del-list"><i class="fa fa-trash"></i></span>' +
            '</div>'+
            '</li>';
        $( '.list-sortable.ingredientDetails-list').append( newMajesticItem );
        $( '.list-sortable.ingredientDetails-list').children("li").slideDown();
        bindMajesticItem();
